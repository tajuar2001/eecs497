...
from userCommunity.community import Community
...
def get_community_recommendations(user_id):
    ...
    for community in Community.query.all():
        community_score = sum(tag.weight for tag in user_tags if tag.name in community.name.lower() or tag.name in community.description.lower())
